  ...
  return (
    <article className="card overflow-hidden transition-shadow hover:shadow-md">
      <Link href={`/news/${news.id}`} className="group block p-5 sm:p-6">
        ...
        <h2 className="text-xl font-bold leading-snug text-ink-900 transition-colors group-hover:text-brand-700 sm:text-2xl dark:text-ink-100 dark:group-hover:text-brand-400">
          {news.title}
        </h2>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-ink-100 pt-3 text-xs text-ink-400 dark:border-ink-800">
          <span className="font-medium text-ink-500 dark:text-ink-300">{news.source}</span>
          <span
            className={cn(
